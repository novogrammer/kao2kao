import Head from 'next/head'
import { useRef ,useState} from 'react';
import { useMount,useUnmount } from "react-use";
import ClientApp from "../client/ClientApp";

export default function Home() {
  const clientAppRef=useRef(null);
  const localVideoRef=useRef(null);
  //TODO n個対応
  const remoteVideoRef=useRef(null);
  const [remoteIsMuted,setRemoteIsMuted]=useState(true);



  useMount(async ()=>{
    const localVideo=localVideoRef.current;
    const remoteVideo=remoteVideoRef.current;
    const clientApp=new ClientApp({
      localVideo,
      remoteVideo,
    });
    window.clientApp=clientApp;
    clientAppRef.current=clientApp;
    await clientApp.setupPromise;
  });
  useUnmount(async ()=>{
    const clientApp=clientAppRef.current;
    await clientApp.destroyAsync();
  });

  return (
    <div>
      <Head>
        <title>Create Next App</title>
        <meta name="description" content="Generated by create next app" />
        {/* <link rel="icon" href="/favicon.ico" /> */}
      </Head>
      <label>remoteIsMuted <input type="checkbox" checked={remoteIsMuted} onClick={()=>setRemoteIsMuted(!remoteIsMuted)}/></label>
      <main>
      <video ref={localVideoRef} playsInline muted />
      <video ref={remoteVideoRef} playsInline muted={remoteIsMuted}/>
      </main>

    </div>
  )
}
